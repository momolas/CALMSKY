...
struct JulianDayTest {
    ...
    func testLocalMidnightWestward() {
        ...
        let longitude1 = 0.0.degrees
        #expect(jd.localMidnight(longitude: longitude1).date == JulianDay(year: 2016, month: 12, day: 20, hour: 0).date)

        let longitude2 = 15.0.degrees // positive = going west, midnight is "later" than UTC hour of input jd
        #expect(jd.localMidnight(longitude: longitude2).date == JulianDay(year: 2016, month: 12, day: 20, hour: 1).date)
        
        let longitude3 = 30.0.degrees
        #expect(jd.localMidnight(longitude: longitude3).date == JulianDay(year: 2016, month: 12, day: 20, hour: 2).date)
        
        let longitude4 = 60.0.degrees // from now on, jump back by one day, degree(60)=hour(4) is greater than input 3hm5s3.5
        #expect(jd.localMidnight(longitude: longitude4).date == JulianDay(year: 2016, month: 12, day: 19, hour: 4).date)
        
        let longitude5 = 90.0.degrees
        #expect(jd.localMidnight(longitude: longitude5).date == JulianDay(year: 2016, month: 12, day: 19, hour: 6).date)

        let longitude6 = 180.0.degrees
        #expect(jd.localMidnight(longitude: longitude6).date == JulianDay(year: 2016, month: 12, day: 19, hour: 12).date)
        
        let longitude7 = 270.0.degrees
        #expect(jd.localMidnight(longitude: longitude7).date == JulianDay(year: 2016, month: 12, day: 19, hour: 18).date)

        let longitude8 = 360.0.degrees // back to Dec 20.
        #expect(jd.localMidnight(longitude: longitude8).date == JulianDay(year: 2016, month: 12, day: 20, hour: 0).date)
    }
    ...
    func testLocalMidnightEastward() {
        ...
        #expect(jd.localMidnight(longitude: longitude2).date == JulianDay(year: 2016, month: 12, day: 19, hour: 23).date)
        
        let longitude3 = -30.0.degrees
        #expect(jd.localMidnight(longitude: longitude3).date == JulianDay(year: 2016, month: 12, day: 19, hour: 22).date)
        
        let longitude4 = -60.0.degrees
        #expect(jd.localMidnight(longitude: longitude4).date == JulianDay(year: 2016, month: 12, day: 19, hour: 20).date)
        
        let longitude5 = -90.0.degrees
        #expect(jd.localMidnight(longitude: longitude5).date == JulianDay(year: 2016, month: 12, day: 19, hour: 18).date)
    
        let longitude6 = -180.0.degrees
        #expect(jd.localMidnight(longitude: longitude6).date == JulianDay(year: 2016, month: 12, day: 19, hour: 12).date)
        
        let longitude7 = -270.0.degrees // From now on, jump forward by one day, since we crossed the line.
        #expect(jd.localMidnight(longitude: longitude7).date == JulianDay(year: 2016, month: 12, day: 20, hour: 6).date)
