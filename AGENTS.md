...
- Prohibit runtime reflection (`String(describing: type(of: self))`) or string parsing for celestial identity and enum resolution in property getters; override open virtual properties with O(1) static literals on concrete classes, memoize heavy physical calculations in immutable `let` properties at initialization, and use pure static engine calls (e.g. `CAAEarth.radiusVector`) instead of heap-allocating temporary domain objects.
- Prohibit escaping buffer pointers (`UnsafeBufferPointer` returned outside `data.withUnsafeBytes`); always encapsulate memory access in scoped closures (`withTableRecord<R>(at:_:)`). In Chebyshev or harmonic series evaluations, vectorize multivariable dot products via Accelerate (`vDSP_dotprD`), simultaneous trigonometric evaluations via `vvsincos`, and coordinate frame rotations via `simd_double3x3` on stack buffers (`withUnsafeTemporaryAllocation`).
- Prohibit disjoint scalar ephemeris calls (e.g. independently calling longitude, latitude, and radiusVector on celestial bodies); resolve full 3D state in a single pass using joint `@inlinable` value DTOs (e.g. `CAAHeliocentricCoordinates` / `PlanetaryHeliocentricCoordinates`) to prevent redundant 3x Kepler/VSOP series evaluations. In iterative light-travel time loops, hoist observer-invariant trigonometric terms ($R \cos(l_0), R \sin(l_0), R \sin(b_0)$) outside the loop, and annotate all core spherical trigonometry transformations and illuminated fraction functions with `@inlinable`.
- In nutation and topocentric calculations: (1) Never evaluate nutation in longitude and obliquity separately (`NutationInLongitude` / `NutationInObliquity`) when both or obliquity are needed; resolve both in a single pass via joint DTO `CAANutationDetails` (`CAANutation.nutationDetails`) to avoid redundant Wahr 1980 evaluations (106 harmonic terms, `vvsincos`, `vDSP_dotprD`). (2) In continuous horizon sampling loops (`TopocentricHorizonEngine` / `RiseTransitSet`), hoist observer invariants outside sampling closures using `FixedEquatorialObserver` for fixed equatorial targets ($\sin\phi\sin\delta, \cos\phi\cos\delta$) and `GlobeEngine.rhoCoordinates` for moving bodies. (3) In diurnal parallax (`CAAParallax`), use exact $\sin\pi = c_1 / \Delta$ instead of $\sin(\arcsin(c_1 / \Delta))$, and cache common denominator terms. (4) For great circle distances (`GreatCircleSeparationEngine.distanceFromGreatArc`), use the normalized vector scalar triple product $\frac{\mathbf{v}_3 \cdot (\mathbf{v}_1 \times \mathbf{v}_2)}{\|\mathbf{v}_1 \times \mathbf{v}_2\|}$ via `simd_double3` to eradicate division by zero at $\cos\alpha_3 = 0$. (5) In binary lookup tables ($\Delta T$), never use `bindMemory(to:)`; always use `loadUnaligned(fromByteOffset:as:)` with explicit bounds checks.
- In eclipses and astrometric vectors: (1) When computing lunation numbers $k$ from dates, always quantify to the exact integer lunation index (`round(K)`) for New Moons and half-integer (`round(K - 0.5) + 0.5`) for Full Moons to prevent fractional phase drift and `modf` failure. (2) In eclipse Besselian element loops (`CAAEclipses.calculate`), reuse base trigonometric evaluations ($m, m', F'$) via register arithmetic ($2\sin\cos$, $\cos^2-\sin^2$, angle addition) instead of invoking redundant transcendent `sin`/`cos` functions. (3) Annotate all vector primitives (`Vector3D`, `StateVector`, `AstrometryReductions`) with `@inlinable`, and support direct SIMD coordinate frame transformations via `simd_double3x3`. (4) Support standardized optical air mass models (`pickering`, `kastenYoung`, `rozenberg`) with `@inlinable` dispatch.
- Prohibit the use of analytical ephemeris theories and Poisson series (such as VSOP2013, VSOP87, ELP2000, or Meeus truncated series) in operational ephemeris pipelines; strictly enforce numerical integration via NASA JPL DE442s (offline baseline) and the multi-agency streaming ensemble (DE442s US, INPOP21a FR, EPM2021 RU, PMOE CN). For French IMCCE ephemerides, exclusively use the INPOP21a numerical SPK integration (`inpop21a.bsp`) which supports HTTP Range streaming and sub-centimetric precision.
- In HTTP byte-range streaming and multi-agency ephemeris evaluation (`StreamingSPKReader`, `SPKRangeClient`): (1) Strictly gate URL schemes to `http`, `https`, or `mock` — never pass `ftp://` or unsupported schemes to `URLSession` range requests to prevent 30-second timeouts. (2) Memoize remote segment failure (`segmentsError`) in the actor so that failing endpoints fail fast (< 0.01 ms) without repeating network calls across bodies or epochs. (3) In asynchronous test or traversal contexts, never use `FileManager.DirectoryEnumerator` (`makeIterator` is unavailable); use `FileManager.default.subpathsOfDirectory(atPath:)` returning a Sendable `[String]` array.
- In SPK numerical ephemeris segment resolution (`SPKEphemerisProvider`, `StreamingSPKEphemerisProvider`), always filter candidates by `$0.dataType == 2` in addition to target and epoch (`$0.targetID == id && $0.dataType == 2 && epoch >= $0.startEpoch && epoch <= $0.endEpoch`) to avoid collision with libration or orientation segments (e.g. Type 20 Euler angle Chebyshev polynomials in EPM2021).
